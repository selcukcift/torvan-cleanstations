...
async function verifyDatabase() {
  ...
  try {
    ...
    // 8. Check Production Tables (NEW) - Only check if they exist
    console.log('\n📋 PRODUCTION TABLES:');
    try {
      ...
      console.log('Production tables not found - run production migration');
    }
    ...
    if (allGood) {
      ...
      console.log('NOTE: Some advanced tables (TaskTemplate, TestProcedureTemplate) may not exist yet.');
      console.log('Run full seeding script to create these if needed.');
    } else {
